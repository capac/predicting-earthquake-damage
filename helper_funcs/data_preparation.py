...
from category_encoders.ordinal import OrdinalEncoder as CEOrdinalEncoder
...
def stratified_shuffle_data_split(prepared_train_values_df, train_labels_df):
    sss = StratifiedShuffleSplit(n_splits=1, test_size=0.3, random_state=42)
    for train_index, val_index in sss.split(prepared_train_values_df, train_labels_df):
        X_strat_train_df = prepared_train_values_df.iloc[train_index]
        y_strat_train_df = train_labels_df.iloc[train_index]
        X_strat_val_df = prepared_train_values_df.iloc[val_index]
    ...
    # X_strat_train, X_strat_val = DataFrame(X_strat_train), DataFrame(X_strat_val)  # type: ignore
    ...
    return X_strat_train_df, y_strat_train_df, X_strat_val_df, y_strat_val_df  # type: ignore
...
def feature_pipeline(train_values_df, num_attrib, cat_attrib):
    ...
    full_pipeline = ColumnTransformer([('num', num_pipeline, num_attrib),
                                       ('cat', CEOrdinalEncoder(), cat_attrib),
                                       ], n_jobs=-1)
